...
import { useResignationList, useApproveResignation } from '@features/misc/hooks';
import { CheckCircle, XCircle } from 'lucide-react-native';

export default function ResignationScreen() {
    const theme = useTheme();
    const toast = useToast();
    const { data: list, isLoading, refetch } = useResignationList();
    const { mutate: approve } = useApproveResignation();
    ...
    return (
        <CorporateBackground>
            <TopBar title="Resignation Approvals" onMenuPress={() => { }} showBack={true} />
                    ...
                    items.map((item: any, index: number) => (
                        <View key={index} style={[styles.card, { backgroundColor: theme.colors.cardPrimary, borderColor: theme.colors.border }]}>
                            <View style={styles.header}>
                                ...
                                <Text style={[styles.date, { color: theme.colors.textSecondary }]}>{item.resignation_date}</Text>
                            </View>
