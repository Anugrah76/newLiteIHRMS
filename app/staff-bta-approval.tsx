...
import { Calendar, CheckCircle, Clock, FileText, List, PlaneIcon, Search, User, XCircle } from 'lucide-react-native';
import { useBTARequests, useApproveBTA } from '@features/staff/api/staffApi';
...
const StatusBadge = ({ status }: { status: string }) => {
    const statusConfig: any = {
        "Submitted": { text: "Submitted", color: "#F59E0B", bg: "#FEF3C7", border: "#FBBF24", icon: Clock },
        "Approved": { text: "Approved", color: "#10B981", bg: "#D1FAE5", border: "#34D399", icon: CheckCircle },
        "Rejected": { text: "Rejected", color: "#EF4444", bg: "#FEE2E2", border: "#F87171", icon: XCircle },
        "Draft": { text: "Draft", color: "#6B7280", bg: "#F3F4F6", border: "#D1D5DB", icon: FileText },
    };
    const config = statusConfig[status] || { text: "Unknown", color: "#6B7280", bg: "#F3F4F6", border: "#9CA3AF", icon: FileText };
...
const RequestCard = ({ item, isSelected, onToggle, theme }: any) => {
    const isPending = item.status === 'Submitted';

    return (
        <TouchableOpacity onPress={isPending ? onToggle : undefined} activeOpacity={0.9}>
            <View style={[styles.card, { backgroundColor: theme.colors.cardPrimary, borderColor: theme.colors.border }, isSelected && { borderColor: theme.colors.primary, borderWidth: 2, backgroundColor: theme.colors.primary + '10' }]}>
                ...
                <View style={styles.cardContent}>
                    ...
                    <View style={styles.cardRow}>
                        ...
                        <View style={styles.cardTextContainer}>
                            <Text style={[styles.cardLabel, { color: theme.colors.textSecondary }]}>Trip Dates</Text>
                            <Text style={[styles.cardValue, { color: theme.colors.text }]}>{item.from_date} → {item.to_date}</Text>
                        </View>
                    </View>
                    <View style={styles.cardRow}>
                        <View style={[styles.cardIconContainer, { backgroundColor: theme.colors.primary + '20' }]}>
                            <PlaneIcon size={18} color={theme.colors.primary} />
                        </View>
                        <View style={styles.cardTextContainer}>
                            <Text style={[styles.cardLabel, { color: theme.colors.textSecondary }]}>Destination</Text>
                            <Text style={[styles.cardValue, { color: theme.colors.text }]}>{item.destination}</Text>
                        </View>
                    </View>
                    <View style={styles.cardRow}>
                        <View style={[styles.cardIconContainer, { backgroundColor: theme.colors.primary + '20' }]}>
                        ...
                        <View style={styles.cardTextContainer}>
                            <Text style={[styles.cardLabel, { color: theme.colors.textSecondary }]}>Purpose</Text>
                            <Text style={[styles.cardValue, { color: theme.colors.text }]} numberOfLines={2}>{item.purpose || "N/A"}</Text>
                        </View>
                    </View>
                </View>
            </View>
...
export default function StaffBTAApprovalScreen() {
    ...
    // React Query hooks
    const searchBTAMutation = useBTARequests();
    const approveBTAMutation = useApproveBTA();
    ...
    const FlatBTList = (data: any[]) => {
        if (!Array.isArray(data)) return [];
        return data.flatMap(employee =>
            employee.businessTrip.map((trip: any) => ({
                ...trip,
                resource_name: employee.resource_name,
                indo_code: employee.indo_code,
                id: `${employee.indo_code}-${trip.bt_id}`
            }))
        );
    };

    const BTRequest = FlatBTList(dataList);
    const pendingRequests = BTRequest.filter(r => r.status === 'Submitted');
    ...
    const handleSearch = async () => {
        ...
        formData.append('approver_id', user.indo_code);
        formData.append('year', year);
        formData.append('month', month);
        formData.append('status', status === "all" ? "" : status);

        try {
            const data = await searchBTAMutation.mutateAsync(formData);
            ...
            console.error('❌ [BTA Search] Error:', error);
    ...
    const handleApprove = async (approvalStatus: string) => {
        ...
        const selectedItems = BTRequest.filter(item => selectedIds.includes(item.id));
        const payload = {
        ...
        try {
            await approveBTAMutation.mutateAsync(payload);
            toast.show('success', approvalStatus === 'Approved' ? 'Approved' : 'Rejected', 'Action completed');
            setSelectedIds([]);
            setMRemark("");
            handleSearch();
        } catch (error: any) {
            console.error('❌ [BTA Approve] Error:', error);
        ...
        <CorporateBackground>
            ...
            <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100, padding: 16 }}>
                <View style={[styles.filterSection, { backgroundColor: theme.colors.cardPrimary }]}>
                    ...
                    <TouchableOpacity
                        style={[styles.searchButton, { backgroundColor: theme.colors.primary }, searchBTAMutation.isPending && { opacity: 0.5 }]}
                        onPress={handleSearch}
                        disabled={searchBTAMutation.isPending}
                    >
                    ...
                    <>
                        <View style={[styles.resultsHeader, { backgroundColor: theme.colors.cardPrimary }]}>
                            <View style={styles.resultsInfo}>
                                ...
                                <Text style={[styles.resultsSubtitle, { color: theme.colors.textSecondary }]}>{BTRequest.length} total • {selectedIds.length} selected</Text>
                            </View>
                        ...
                        {selectedIds.length > 0 && (
                            <View style={[styles.actionSection, { backgroundColor: theme.colors.cardPrimary }]}>
                                ...
                                <View style={styles.actionButtons}>
                                    <TouchableOpacity
                                        style={[styles.actionButton, styles.rejectButton, approveBTAMutation.isPending && { opacity: 0.5 }]}
                                        onPress={() => handleApprove("Rejected")}
                                        disabled={approveBTAMutation.isPending}
                                    >
                                        <XCircle size={20} color="#FFFFFF" />
                                        <Text style={styles.actionButtonText}>Reject ({selectedIds.length})</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        style={[styles.actionButton, styles.approveButton, approveBTAMutation.isPending && { opacity: 0.5 }]}
