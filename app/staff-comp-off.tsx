...
import { useCompOffRequests, useApproveCompOff } from '@features/staff/api/staffApi';
            ...
            <View style={[styles.card, { backgroundColor: theme.colors.cardPrimary, borderColor: theme.colors.border }, isSelected && { borderColor: theme.colors.primary, borderWidth: 2, backgroundColor: theme.colors.primary + '10' }]}>
                ...
                <View style={styles.cardContent}>
                    ...
                    <View style={styles.cardRow}>
                        ...
                        <View style={styles.cardTextContainer}>
                            <Text style={[styles.cardLabel, { color: theme.colors.textSecondary }]}>Comp-Off Date</Text>
                            <Text style={[styles.cardValue, { color: theme.colors.text }]}>{item.compoff_date}</Text>
                        </View>
                    </View>
                </View>
            </View>
...
export default function StaffCompOffScreen() {
    ...
    // React Query hooks
    const searchCompOffMutation = useCompOffRequests();
    const approveCompOffMutation = useApproveCompOff();
    ...
    const flattenCompOffList = (data: any[]) => {
        if (!Array.isArray(data)) return [];
        return data.flatMap(employee =>
            employee.compoff.map((request: any) => ({
                ...request,
                resource_name: employee.resource_name,
                indo_code: employee.indo_code,
                id: `${employee.indo_code}-${request.compoff_id}`
            }))
        );
    };

    const compoffRequests = flattenCompOffList(dataList);
    const pendingRequests = compoffRequests.filter(r => r.status === '1' || r.status === 1);
    ...
    const handleSearch = async () => {
        ...
        try {
            const data = await searchCompOffMutation.mutateAsync(formData);
            ...
            console.error('❌ [CompOff Search] Error:', error);
    ...
    const handleApprove = async (stat: number) => {
        ...
        const selectedItems = compoffRequests.filter(r => selectedIds.includes(r.id));
        const payload = {
            compoffList: selectedItems.map(item => ({
                compoff_id: item.compoff_id,
                indo_code: item.indo_code || "",
                compoff_date: item.compoff_date || "",
                status: stat
            })),
            key: user?.api_key || "",
        ...
        try {
            await approveCompOffMutation.mutateAsync(payload);
            toast.show('success', stat === 3 ? 'Approved' : 'Rejected', 'Action completed');
            setSelectedIds([]);
            setMRemark("");
            handleSearch();
        } catch (error: any) {
            console.error('❌ [CompOff Approve] Error:', error);
        ...
        <CorporateBackground>
            ...
            <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100, padding: 16 }}>
                ...
                <View style={[styles.filterSection, { backgroundColor: theme.colors.cardPrimary }]}>
                    ...
                    <TouchableOpacity
                        style={[styles.searchButton, { backgroundColor: theme.colors.primary }, searchCompOffMutation.isPending && { opacity: 0.5 }]}
                        onPress={handleSearch}
                        disabled={searchCompOffMutation.isPending}
                    >
                ...
                {/* Results */}
                {searchCompOffMutation.isPending ? (
                    ...
                    <>
                        <View style={[styles.resultsHeader, { backgroundColor: theme.colors.cardPrimary }]}>
                            <View style={styles.resultsInfo}>
                                ...
                                <Text style={[styles.resultsSubtitle, { color: theme.colors.textSecondary }]}>{compoffRequests.length} total • {selectedIds.length} selected</Text>
                            </View>
                        ...
                        {selectedIds.length > 0 && (
                            <View style={[styles.actionSection, { backgroundColor: theme.colors.cardPrimary }]}>
                                ...
                                <View style={styles.actionButtons}>
                                    <TouchableOpacity
                                        style={[styles.actionButton, styles.approveButton, approveCompOffMutation.isPending && { opacity: 0.5 }]}
                                    ...
                                    <TouchableOpacity
                                        style={[styles.actionButton, styles.rejectButton, approveCompOffMutation.isPending && { opacity: 0.5 }]}
