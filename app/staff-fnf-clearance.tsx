...
import { Calendar, CheckCircle, Clock, FileText, List, Search, User, XCircle } from 'lucide-react-native';
import { useFNFRequests, useApproveFNF } from '@features/staff/api/staffApi';
...
export default function StaffFnFClearanceScreen() {
    ...
    // React Query hooks
    const searchFNFMutation = useFNFRequests();
    const approveFNFMutation = useApproveFNF();
    ...
    const FFRequest = FlatLeaveList(dataList);
    const pendingRequests = FFRequest.filter(r => r.status === '1' || r.status === 1);
    ...
    const handleSearch = async () => {
        ...
        if (status !== "all") formData.append('status', status);

        try {
            const data = await searchFNFMutation.mutateAsync(formData);
            ...
            console.error('❌ [F&F Search] Error:', error);
    ...
    const handleApprove = async (stat: number) => {
        ...
        const selectedItems = FFRequest.filter(item => selectedIds.includes(item.id));
        const payload = {
            leaveList: selectedItems.map(item => ({
            ...
            approver_mail: user?.email || "",
        };

        try {
            // staffApi.approveFNF handles the wrapping in "rawData"
            await approveFNFMutation.mutateAsync(payload);
            toast.show('success', stat === 3 ? 'Approved' : 'Rejected', 'Action completed');
            setSelectedIds([]);
            setMRemark("");
            handleSearch(); // Refresh
        } catch (error: any) {
            console.error('❌ [F&F Approve] Error:', error);
        ...
        <CorporateBackground>
            ...
            <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100, padding: 16 }}>
                <View style={[styles.filterSection, { backgroundColor: theme.colors.cardPrimary }]}>
                    ...
                    <TouchableOpacity
                        style={[styles.searchButton, { backgroundColor: theme.colors.primary }, searchFNFMutation.isPending && { opacity: 0.5 }]}
                        onPress={handleSearch}
                        disabled={searchFNFMutation.isPending}
                    >
                    ...
                    <>
                        <View style={[styles.resultsHeader, { backgroundColor: theme.colors.cardPrimary }]}>
                            <View style={styles.resultsInfo}>
                                ...
                                <Text style={[styles.resultsSubtitle, { color: theme.colors.textSecondary }]}>{FFRequest.length} total • {selectedIds.length} selected</Text>
                            </View>
                        ...
                        {selectedIds.length > 0 && (
                            <View style={[styles.actionSection, { backgroundColor: theme.colors.cardPrimary }]}>
                                ...
                                <View style={styles.actionButtons}>
                                    <TouchableOpacity
                                        style={[styles.actionButton, styles.rejectButton, approveFNFMutation.isPending && { opacity: 0.5 }]}
                                        onPress={() => handleApprove(4)}
                                        disabled={approveFNFMutation.isPending}
                                    >
                                        <XCircle size={20} color="#FFFFFF" />
                                        <Text style={styles.actionButtonText}>Reject ({selectedIds.length})</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        style={[styles.actionButton, styles.approveButton, approveFNFMutation.isPending && { opacity: 0.5 }]}
